...
module.exports = async (client, message, type, victim, reason) => {
  ...
  try {
    ...
    let caseid;
    try {
      caseid = await sql.query(
        `SELECT MAX(caseid) FROM \`${message.guild.id}\``
      );
    ...
    try {
      await sql.query(
        `INSERT INTO \`${message.guild.id}\` (caseid, reason, victim, moderator, date, type, link) VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [caseid, reason, victim, mod, epoch, type, msglink]
      );
    } catch (err) {
      console.log(err);
    }
