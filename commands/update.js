...
exports.run = async (client, message, args) => {
    ...
    caseid = args[0]
    let things = await sql.query(`SELECT caseid, reason, updates, victim, moderator, date, type, link FROM \`${message.guild.id}\` WHERE caseid=${caseid}`)
    if (!things[0]) {
    ...
    try {
        ...
        console.log(err)
        message.channel.send(`I cannot update the embed and thus cannot update the case. Ensure that I have permission to send messages in <#${logchannel}>, and that your updates aren't too long.`)
        return;
    }

    try {
        await sql.query(`UPDATE \`${message.guild.id}\` SET updates = '${pleasework}' WHERE caseid=${caseid}`)
    } catch (err) {
        mesag.edit(ogembed)
        console.log(err)
        return;
