...
exports.run = async (client, message, args) => {
  ...
  try {
    ...
    if (!message.guild.member(uid).roles.cache.keyArray().includes(muterole)) {
      message.channel.send(
        `**${message.guild.member(uid).user.tag}** is already unmuted.`
      );
      return;
    }
    args = args.splice(1);
    if (
      ...
      message.channel.send(
        "🚫 You do not have permission to unmute that user."
      );
      return;
    }
  } catch (err) {
  ...
  try {
    ...
    message.channel.send(
      `${cid[0]}User **${member}** has been unmuted. (Case ${cid[1]})`
    );
  } catch (err) {
    console.log(err);
    message.channel.send(
      "Ensure your message is in the format `unmute userid (reason)`"
    );
  }
};
