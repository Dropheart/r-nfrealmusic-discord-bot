  ...
  try {
    ...
    if (message.guild.member(uid).roles.cache.keyArray().includes(muterole)) {
      message.channel.send(
        `**${message.guild.member(uid).user.tag}** is already muted.`
      );
      return;
    }
    args = args.splice(1);
    ...
    if (err.message == "Cannot read property 'fetch' of null") {
      ...
      message.channel.send(
        "Ensure your message is formatted as such: `mute userid (time) (reason)`"
      );
    }
    return;
  }
  ...
  try {
    if (args[0].match(/^(\d)*([smhdy]|(hr))$/g)) {
      ...
      if (time.endsWith("h")) {
        ...
        var epoch =
          parseInt(Math.floor(new Date() / 1000)) + parseInt(time.slice(0, -1));
        time = time.slice(0, -1) + " second(s)";
      } else if (time.endsWith("m")) {
        var epoch =
          parseInt(Math.floor(new Date() / 1000)) +
          parseInt(time.slice(0, -1)) * 60;
      ...
      let fsread = fs.readFileSync(
        `./timers/mutes/${message.guild.id}.yml`,
        "utf8"
      );
      let data = {
        [uid]: epoch,
    ...
    if (time) {
      ...
      try {
        ...
        message.channel.send(
          `🤐 ${cid[0]}User **${member}** has been muted for ${time}. (Case ${cid[1]})`
        );
      } catch (err) {
        message.channel.send(
    ...
    try {
      ...
      canirun(message, false, "mod");
      message.channel.send(
        `🤐 ${cid[0]}User **${member}** has been muted indefinitely. (Case ${cid[1]})`
      );
    } catch (err) {
      message.channel.send(
        "🚫 I do not have permission to assign the mute role."
