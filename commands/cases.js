...
exports.run = async (client, message, args) => {
  ...
  let total = await sql.query(
    `SELECT MAX(caseid) FROM \`${message.guild.id}\` WHERE victim=${uid}`
  );
  ...
  embed = new MessageEmbed().setAuthor(
    `Cases for ${myguy.tag} (${total[0]["MAX(caseid)"]} total)`,
    myguy.displayAvatarURL()
  );
